export const navLinks = [
    {
        ger: "Produkte",
        en: "Product",
        dutc: "Product",
        fr: "Produit",
        jp: "製品",
        per: 'محصولات',
        url: "#"
    },
    {
        ger: "Kontakt",
        en: "Contact",
        dutc: "Contact",
        fr: "Contact",
        jp: "接触",
        per: 'ارتباط',
        url: "#"
    },
    {
        ger: "Karriere",
        en: "Career",
        dutc: "Carrière",
        fr: "Carrière",
        jp: "キャリア",
        per: 'همکاری',
        url: "#"
    },
    {
        ger: "Über Uns",
        en: "About Us",
        dutc: "Over Ons",
        fr: "À propos de nous",
        jp: "私たちについて",
        per: 'درباره ما',
        url: "#"
    },
]

export const yourNotes = {
    yourNote: {
        ger: "Ihre Notizen",
        en: "Your Notes",
        dutc: "Jouw Notities",
        fr: "Vos notes",
        jp: "あなたのメモ",
        per: "یادداشت شما",
    },

    yourFeeling: {
        ger: "Ihre Gefühle",
        en: "Your Feelings",
        dutc: "Jouw Gevoelens",
        fr: "Vos sentiments",
        jp: "あなたの気持ち",
        per: "احساسات شما",
    },

    paragraph: {
        ger: "Nehmen Sie Ihren Bleistift, spüren Sie die Luft, schreiben Sie Ihr Leben auf und malen Sie Ihre Fantasie",
        en: "Take your pencil, feel the air, write down your life and paint your imagination",
        dutc: "Neem je potlood, voel de lucht, schrijf je leven op en schilder je verbeelding",
        fr: "Prenez votre crayon, sentez l'air, écrivez votre vie et peignez votre imagination",
        jp: "鉛筆を手に取り、空気を感じ、あなたの人生を書き留め、あなたの想像力を描きましょう",
        per: "یه مداد بردار، یه نفس عمیق، زندگیت رو یا بنویس، یا نقاشی بکش"
    }
}

export const purposes = {
    purpose: {
        ger: "Für alle Einsatzzwecke",
        en: "For All Purposes",
        dutc: "Voor alle Doeleinden",
        fr: "Pour tous les usages",
        jp: "あらゆる用途に",
        per: "برای همه هدف ها"
    },

    paragraph: {
        ger: "Jedes Notizbuch ist eine Geschichte, die Sie erzählen können, jedes Notizbuch ist eine neue Welt, die Sie sehen können",
        en: "Every notebook is a story you can tell, every notebook is a new world you can see",
        dutc: "Elk notitieboekje is een verhaal dat je kunt vertellen, elk notitieboekje is een nieuwe wereld die je kunt zien",
        fr: "Chaque carnet est une histoire que vous pouvez raconter, chaque carnet est un nouveau monde que vous pouvez voir",
        jp: "すべてのノートはあなたが語ることができる物語であり、すべてのノートはあなたが見ることができる新しい世界です",
        per: "هر دفترچه یادداشتی یک داستان برای گفتن داره، و هرکدوم یک دنیایی برای دیدن"
    },

    purposeList: [
        {
            ger: "Erweitern Sie die Ideen",
            en: "Expand the ideas",
            dutc: "Breid de ideeën uit",
            fr: "Développez les idées",
            jp: "アイデアを広げる",
            per: "گسترش ایده ها",
            icon: `<img src="./assets/images/lightbulb.webp" height="35">`
        },

        {
            ger: "Bringen Sie Kunst mit",
            en: "Bring Art With You",
            dutc: "Breng kunst mee",
            fr: "Apportez de l'art avec vous",
            jp: "アートを持ち歩きましょう",
            per: "هنرت رو همراه داشته باش",
            icon: `<img src="./assets/images/palette.webp" height="35">`
        },

        {
            ger: "Verwirklichen Sie die Daten",
            en: "Make Your Notes Reality",
            dutc: "Maak uw aantekeningen werkelijkheid",
            fr: "Faites de vos notes une réalité",
            jp: "メモを現実にする",
            per: "یادداشت رو به واقعیت تبدیل کن",
            icon: `<img src="./assets/images/magic.webp" height="35">`
        },

        {
            ger: "Auf der Reise",
            en: "On the Journey",
            dutc: "Op reis",
            fr: "En voyage",
            jp: "旅の途中",
            per: "برای سفر",
            icon: `<img src="./assets/images/planeicon.webp" height="35">`
        },

        {
            ger: "Im Unterricht",
            en: "In the Class",
            dutc: "In de klas",
            fr: "En classe",
            jp: "授業中",
            per: "برای کلاس",
            icon: `<img src="./assets/images/class.webp" height="35">`
        },

        {
            ger: "Im Zuhause",
            en: "At Home",
            dutc: "Thuis",
            fr: "À la maison",
            jp: "自宅で",
            per: "برای خانه",
            icon: `<img src="./assets/images/home.webp" height="35">`
        },
    ]
}

export const diary = {
    ger: "Lesen Sie mein Tagebuch",
    en: "Read My Diary",
    dutc: "Lees mijn dagboek",
    fr: "Lire mon journal",
    jp: "私の日記を読んでください",
    per: "نوشته های منم ببین",
}

export const slides = [
    {
        title_en: "History",
        title_ger: "Geschichte",
        title_dutc: "Geschiedenis",
        title_fr: "Histoire",
        title_jp: "歴史",
        title_per: "تاریخ",

        description_en: "Your words make history",
        description_ger: "Deine Worte schreiben Geschichte",
        description_dutc: "Jouw woorden maken geschiedenis",
        description_fr: "Tes mots écrivent l'histoire",
        description_jp: "あなたの言葉が歴史を作る",
        description_per: "کلمات تو تاریخ می‌سازند",

        img: "./assets/images/slide1.webp"
    },

    {
        title_en: "Existance",
        title_ger: "Existenz",
        title_dutc: "Existenz",
        title_fr: "existence",
        title_jp: "存在",
        title_per: "وجود",

        description_en: "Existence Precedes Essence",
        description_ger: "Die Existenz geht der Essenz voraus",
        description_dutc: "Het bestaan gaat vooraf aan de essentie",
        description_fr: "L'existence précède l'essence",
        description_jp: "実存は本質に先立つ",
        description_per: "وجود بر ماهیت تقدم دارد",

        img: "./assets/images/slide2.webp"
    },

    {
        title_en: "Sempiternal",
        title_ger: "Ewigkeit",
        title_dutc: "Eeuwigdurend",
        title_fr: "Sempiternel",
        title_jp: "永遠の",
        title_per: "جاودانگی",

        description_en: "It's sempiternal when you write it down",
        description_ger: "Es ist sempiternal, wenn du es aufschreibst",
        description_dutc: "Het is sempitern wanneer je het opschrijft.",
        description_fr: "C'est sempiternel quand tu l'écris.",
        description_jp: "かきとめると、それはえいえんです。",
        description_per: "وقتی آن را می‌نویسی، جاودانه است",

        img: "./assets/images/slide3.webp"
    },
]

export const hearYou = {
    weHearYou: {
        ger: "Wir würden gerne von Ihnen hören",
        en: "We Would Love To Hear From You",
        dutc: "Wij Horen Graag van U",
        fr: "Nous aimerions avoir de vos nouvelles",
        jp: "ご意見をお聞かせください",
        per: "ما مشتاقیم از شما بشنویم",
    },

    paragraph: {
        ger: "fiktive Geschichte, Lebenstagebuch, Kunst, was auch immer Sie aufgeschrieben haben. Gewinnen Sie in unserem Turnier!",
        en: "Fictional story, life diary, art, whatever you've written down. Win in our tournament!",
        dutc: "Fictief verhaal, levensdagboek, kunst, wat je ook maar hebt opgeschreven. Win in ons toernooi!",
        fr: "Histoire fictive, journal intime, œuvre d'art, peu importe ce que vous avez écrit. Gagnez notre tournoi !",
        jp: "架空の物語、日記、アートなど、あなたが書き留めたものなら何でも。トーナメントで優勝しよう！",
        per: "داستان نویسی تخیلی، خاطرات، هرچیزی که نوشتی. تورنومنت ما رو برنده شو!",
    }
}

export const ourGallery = {
    ourGallery: {
        ger: "Unsere Galerie",
        en: "Our Gallery",
        dutc: "Onze Galerij",
        fr: "Notre galerie",
        jp: "私たちのギャラリー",
        per: "گالری ما"
    },

    paragraph: {
        ger: "Ihre Reise wird in unserer Galerie festgehalten, um sie der Welt zu zeigen",
        en: "Your journey will be captured in our gallery to share with the world",
        dutc: "Uw reis wordt vastgelegd in onze galerij om met de wereld te delen",
        fr: "Votre voyage sera capturé dans notre galerie pour être partagé avec le monde",
        jp: "あなたの旅は私たちのギャラリーに記録され、世界と共有されます",
        per: "ماجرای شما در گالری ما ثبت خواهد شد"
    }
}

export const statics = [
    {
        number: "15",
        ger: "Städte",
        en: "Cities",
        dutc: "Steden",
        fr: "Villes",
        jp: "都市",
        per: "شهرها"
    },
    {
        number: "20",
        ger: "Zweige",
        en: "Branches",
        dutc: "Takken",
        fr: "Succursales",
        jp: "支店",
        per: "شعبه"
    },
    {
        number: "3000",
        ger: "Kunden",
        en: "Customers",
        dutc: "Klanten",
        fr: "Clientes/Clients",
        jp: "お客様",
        per: "مشتری"
    },
    {
        number: "+1000",
        ger: "Produkte",
        en: "Products",
        dutc: "Producten",
        fr: "Produits",
        jp: "製品",
        per: "محصولات"
    },
]

export const gallery = [
    {
        dataAOS: "fade-zoom-in",
        img: "./assets/images/a.webp"
    },
    {
        dataAOS: "fade-zoom-in",
        img: "./assets/images/h.webp"
    },
    {
        dataAOS: "fade-right",
        img: "./assets/images/d.webp"
    },
    {
        dataAOS: "fade-left",
        img: "./assets/images/f.webp"
    },
    {
        dataAOS: "fade-zoom-in",
        img: "./assets/images/e.webp"
    },
    {
        dataAOS: "fade-zoom-in",
        img: "./assets/images/g.webp"
    },
]

export const preFooter = [
    "./assets/images/i.webp",
    "./assets/images/l.webp",
    "./assets/images/j.webp",
]